                ...
                <tbody>
                  {row.ranked.map((m) => (
                    <tr key={m.attacker.pokemon.id + '-' + m.attacker.level} className="border-t border-stone-100 dark:border-stone-800">
                      ...
                      <td className="py-1">{m.best ? <>{m.best.move.name} <EffChip mult={m.best.typeMult} /> {pct(m.best.minPct, m.best.maxPct)}</> : '—'}</td>
                      <td className="py-1 text-stone-500">{m.threat ? `takes ${pct(m.threat.minPct, m.threat.maxPct)} (${m.threat.move.name})` : 'takes nothing'}</td>
